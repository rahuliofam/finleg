...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/sessions') {
      ...
      if (search) {
        // Split into terms, respecting quoted phrases for exact matching
    ...
    if (request.method === 'POST' && url.pathname === '/sessions/ask') {
      ...
      if (body.session_ids?.length) {
        ...
        const searchTerms = parseSearchTerms(body.search);
        const searchConditions = searchTerms.map(() => '(summary LIKE ? OR transcript LIKE ?)').join(' AND ');
        const searchParams = searchTerms.flatMap(t => [`%${t}%`, `%${t}%`]);
        const result = await env.DB.prepare(
...
// Parse search string into terms, respecting quoted phrases
// e.g. '"cat in the hat" schwab' → ['cat in the hat', 'schwab']
