    ...
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: 'Extract structured metadata from this thought. Return ONLY valid JSON, no markdown:\n{\n  "type": "person_note|action_item|insight|observation|decision|question",\n  "tags": ["category1", "category2"],\n  "people": ["name1"],\n  "action_items": ["task"],\n  "priority": "high|medium|low"\n}',
          },
...
const TOOLS = [
  {
    ...
    description: "Semantic search across all captured thoughts. Returns the most relevant matches based on meaning, not just keywords.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "stats",
    description: "Get statistics about the thought database — total count, date range.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "capture_thought",
    description: "Capture a new thought directly into the brain. Use this to save insights, notes, decisions, or anything worth remembering.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        try {
          ...
          switch (toolName) {
            case "search_thoughts":
              content = await searchThoughts(args.query, args.threshold, args.limit);
              break;
            case "browse_recent":
