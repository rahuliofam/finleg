...
async function forwardEmail(
  ...
  const fromLine = original.from?.[0]
    ? `${original.from[0].name || ""} <${original.from[0].address}>`.trim()
    : "unknown sender";

  const forwardHtml = `
    <p><strong>Forwarded email to @finleg.net</strong></p>
    <p><strong>From:</strong> ${fromLine}<br>
    <strong>To:</strong> ${original.to?.map((t: any) => t.address).join(", ") || "?"}<br>
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `Email from: ${email.from?.[0]?.address}, to: ${email.to?.[0]?.address}, subject: ${email.subject}`
    );
