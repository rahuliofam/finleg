...
export default function GeneralFilesTab() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto">
      ...
      <div className="flex items-baseline gap-4 mb-6 flex-wrap">
        <h1 className="text-2xl font-bold text-slate-900">General Files</h1>
        {stats?.total_files != null && (
          <span className="text-sm text-slate-400">
            {Number(stats.total_files).toLocaleString()} files &bull; {stats.total_size_tb || "?"} TB
          </span>
        )}
      ...
      {!driveOnline && (
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 mb-4 text-sm flex items-center gap-2">
          <span>&#9888;</span>
          <span>External drive is offline — search works but previews are unavailable.</span>
        </div>
      )}
      ...
      {!searching && !searchError && results.length === 0 && total === 0 && (
        <div className="text-center py-12 text-slate-400">
          ...
          <p>Search all files. Enter a name or browse by type.</p>
        </div>
      )}
