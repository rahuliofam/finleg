...
async function sendConflictEmail(filename, conflicts, gemini, claude) {
  ...
  html += `<div style="font-weight:600;margin-bottom:8px;">📄 ${filename}</div>`;
  ...
  for (const c of conflicts) {
    ...
    html += `<td style="padding:4px 8px;font-family:monospace;font-size:12px;">${c.field}</td>`;
    html += `<td style="padding:4px 8px;text-align:right;">${fmtG}</td>`;
    html += `<td style="padding:4px 8px;text-align:right;">${fmtC}</td>`;
    html += `<td style="padding:4px 8px;text-align:right;color:#dc2626;">${fmtD}</td>`;
    html += `</tr>`;
  }
  html += `</table>`;

  html += `<p style="font-size:13px;color:#666;">The Gemini extraction was used as the primary source. Review the conflicts above and update the database if Claude's values are more accurate.</p>`;
  html += `</div>`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.log(`    ✉ Conflict email sent to ${NOTIFY_TO}`);
    }
  } catch (err) {
